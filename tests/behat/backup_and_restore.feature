@qtype @qtype_proforma
Feature: BACKUP AND RESTORE
  Test duplicating a quiz containing a ProFormA question
  As a teacher
  In order re-use my courses containing ProFormA questions
  I need to be able to backup and restore them

  Background:
    Given the following "users" exist:
      | username | firstname | lastname | email               |
      | teacher1 | T1        | Teacher1 | teacher1@moodle.com |
    And the following "courses" exist:
      | fullname | shortname | category |
      | Course 1 | C1        | 0        |
    And the following "question categories" exist:
      | contextlevel | reference | name           |
      | Course       | C1        | Test questions |
    And the following "questions" exist:
      | questioncategory | qtype     | name      | template         |
      | Test questions   | proforma     | proforma-001 | editor           |
      | Test questions   | proforma     | proforma-003 | filepicker            |
    And the following "activities" exist:
      | activity   | name      | course | idnumber |
      | quiz       | Test quiz | C1     | quiz1    |
    And quiz "Test quiz" contains the following questions:
      | proforma-001 | 1 |
      | proforma-003 | 1 |
    And I log in as "admin"
    And I am on "Course 1" course homepage

  @javascript
  Scenario: Backup and restore a course containing 2 ProFormA questions
    When I backup "Course 1" course using this options:
      | Confirmation | Filename | test_backup.mbz |
    And I restore "test_backup.mbz" backup into a new course using this options:
      | Schema | Course name | Course 2 |
    And I navigate to "Question bank" node in "Course administration"
    And I should see "proforma-001"
    And I should see "proforma-003"
    And I click on "Edit" "link" in the "proforma-001" "table_row"
    Then the following fields match these values:
      | Question name              | proforma-001                                               |
      | Question text              | Please code the reverse string function not using a library function.(äöüß)                      |
      | Default mark               | 1                  |  
      | General feedback           | You must not use a library function. |

## internal storage => task path and repository are not shown
##      | Task path                  | /path/to/reversestring.zip |
##      | Repository                 | https://repository.ostfalia.de                            |
#      | Response filename           | MyString.java                                       |
#      | UUID                       | UUID 1                                              |
      | Syntax highlighting        | java                                                |
      | Response format            | Editor                                              |
      | Input box size             | 10 lines                                            |
# cannot read data from codemirror???
#      | Response template | //text in responsetemplate (äöüß)                   |
      | Comment    | Check if the code uses a library function.    |
      | Penalty for each incorrect try    | 20%                      |
## TODO: check templates, libraries and model solution
#      | templates                         | xxx                      |
#    | libraries                         | xxx                      |

#      | Hint 1                     | hint 1<br>                |
#      | Hint 2                     | hint 2<br>                    |
#    And I should see "UUID 1"
    And I press "Cancel"
    And I click on "Edit" "link" in the "proforma-003" "table_row"
    Then the following fields match these values:
      | Question name              | proforma-003                                               |
      | Question text              | Please code the reverse string function not using a library function.(äöüß)                      |
      | Default mark               | 1                  |
      | General feedback           | You must not use a library function.|
##      | Task path                  | /path/to/reversestring.zip |
##      | Repository                 | https://repository.ostfalia.de                            |
#      | Response filename           | MyString.java                                       |
#      | UUID                       | UUID 1                                              |
      | Syntax highlighting        | python                                               |
      | Response format            | File picker                                     |
      | Max. number of uploaded files | 3                                             |
      | Max. upload size          | 10KB                                                   |
      | Accepted file types              | .java, .jar                                          |
# cannot read data from codemirror???
#      | Response template          | //text in responsetemplate (äöüß)                   |
      | Comment    | Check if the code uses a library function.     |
      | Penalty for each incorrect try    | 20%                      |
#      | Hint 1                     | hint 1<br>                |
#      | Hint 2                     | hint 2<br>                    |
#    And I should see "UUID 1"
    And I press "Cancel"
