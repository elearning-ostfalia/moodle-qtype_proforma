...
class qtype_proforma_question extends question_graded_automatically {
    ...
    public function extract_data_from_taskfile() {
        ...
        $taskfile = $this->get_task_file();

        if (!$taskfile instanceof stored_file) {
            throw new coding_exception("no task file found");
        }
        ...
        try {
            $files = $taskfile->extract_to_pathname(get_file_packer('application/zip'), $tempdir);
            if (!$files)
        ...
        list ($tests, $gradinghints) = qtype_proforma::extract_data_from_taskfile($contents);
        // merge grading hints
        if (isset($this->gradinghints)) {
            $xmldoc = new DOMDocument;

            if (!$xmldoc->loadXML($this->gradinghints)) {
                debugging('gradinghints is not valid XML');
                return array($tests, $gradinghints); // 'INTERNAL ERROR: $taskresult is not XML';
            }
            ...
            if ($xpathresult->length > 0) {
                foreach ($xpathresult as $test) {
                    $ref = $test->getAttribute('ref');
                    $titles = $xpath->query('title', $test);
                    if (array_key_exists($ref, $gradinghints)) {
                        $testobject = $gradinghints[$ref];
                        $testobject['ref'] = $ref;
